...
public class MasterSlaveMode implements MRedisTemplate {
   ...
   public MasterSlaveMode(Client masterSlaveClient) {
      this.redisURI = RedisURI.builder().withHost(masterSlaveClient.getHost()).withPort(masterSlaveClient.getPort()).withPassword(masterSlaveClient.getPassword().toCharArray()).withDatabase(masterSlaveClient.getDatabase()).withTimeout(Duration.ofMillis(30000L)).build();
